...
bool FriendListCommand::execute(std::vector<std::string>* args) {
	...
	if (playerName.size() <= 1) {
		clientMessageF("[%sHijacker%s] %sCouldn't find player: %s!", DARK_BLUE, WHITE, RED, searchedName.c_str());
		return true;
	}
	if (subcommand == "add") {
		FriendList::addPlayerToList(playerName);
		clientMessageF("[%sHijacker%s] %s%s is now your friend!", DARK_BLUE, WHITE, GREEN, playerName.c_str());
		return true;

	} else if (subcommand == "remove") {
		if (FriendList::removePlayer(searchedName)) {
			clientMessageF("[%sHijacker%s] %s%s has been removed from your friendlist!", DARK_BLUE, WHITE, GREEN, searchedName.c_str());
			return true;
		} else {
			clientMessageF("[%sHijacker%s] %s%s was not in your friendlist!", DARK_BLUE, WHITE, GREEN, searchedName.c_str());
			return true;
		}
	}
	return true;
