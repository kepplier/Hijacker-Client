...
bool NameSpoofCommand::execute(std::vector<std::string>* args) {
	if (args->at(1) == "name" && args->size() > 2) {
		...
		clientMessageF("[%sHijacker%s] %sSet fakename to %s%s%s, please reconnect!", DARK_BLUE, WHITE, GREEN, GRAY, name->getText(), GREEN);
		return true;
	} else if (args->at(1) == "reset") {
		Game.setFakeName(NULL);
		clientMessageF("[%sHijacker%s] %sReset fakename!", DARK_BLUE, WHITE, GREEN);
		return true;
	}
	return false;
}
