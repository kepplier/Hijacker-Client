...
bool PlayerTeleportCommand::execute(std::vector<std::string>* args) {
	...
	if (pos.iszero()) {
		clientMessageF("[%ssHijacker%s] %sCouldn't find player: %s!", DARK_BLUE, WHITE, RED, nameOfPlayer.c_str());
		return true;
	}
	Game.getLocalPlayer()->setPos(pos);
	clientMessageF("[%sHijacker%s] %sTeleported to %s", DARK_BLUE, WHITE, GREEN, playerName.c_str());
	return true;
}
