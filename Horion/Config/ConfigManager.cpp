...
void ConfigManager::loadConfig(std::string name, bool create) {
	...
	if (configExists || create) {
		...
		if (Game.getLocalPlayer() != nullptr) {
			...
			Game.getGuiData()->displayClientMessageF("[%sHijacker%s] %sSuccessfully %s config %s%s%s!", DARK_BLUE, WHITE, GREEN, !configExists ? "created" : "loaded", GRAY, name.c_str(), GREEN);
			if (!helpedUser && name != "default") {
				helpedUser = true;
				Game.getGuiData()->displayClientMessageF("[%sHijacker%s] %sEnter \"%s%cconfig load default%s\" to load your old config!", DARK_BLUE, WHITE, YELLOW, WHITE, cmdMgr->prefix, YELLOW);
			}
		}
	} else {
		if (Game.getLocalPlayer() != nullptr) 
			Game.getGuiData()->displayClientMessageF("[%sHijacker%s] %sCould not load config %s%s%s!", DARK_BLUE, WHITE, RED, GRAY, name.c_str(), RED);
	}
