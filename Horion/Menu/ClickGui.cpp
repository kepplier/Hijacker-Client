...
static const MC_Color Checkbox = MC_Color(255, 255, 255);
...
void ClickGui::renderCategory(Category category) {
	...
	if (ourWindow->isExtended || ourWindow->isInAnimation) {
		...
		for (auto& mod : moduleList) {
			...
			// Text
			if (allowRender)
				DrawUtils::drawText(textPos, &textStr, mod->isEnabled() ? whiteColor : MC_Color(255,255,255), textSize);
						...
						for (auto setting : *settings) {
							...
							case ValueType::BOOL_T: {
								...
								// Checkbox
								{
									...
									DrawUtils::drawRectangle(boxPos, Checkbox, isFocused ? 1 : 0.8f, 0.5f);

									if (setting->value->_bool) {
										DrawUtils::setColor(255, 119, 218, 1);
										boxPos.x += 1;
										boxPos.y += 1;
										boxPos.z -= 1;
										boxPos.w -= 1;
								...
								// Text
								{
									...
									DrawUtils::drawText(textPos, &elTexto, isFocused ? whiteColor : MC_Color(200, 200, 200), textSize);
									currentYOffset += textHeight + (textPaddingY * 2);
								}
								break;
							}
...
void ClickGui::render() {
	...
	// Fill Background
	DrawUtils::fillRectangle(Vec4(0, 0, Game.getClientInstance()->getGuiData()->widthGame, Game.getClientInstance()->getGuiData()->heightGame), MC_Color(255, 119, 218), 0.f);
