...
void Hooks::ClientInstanceScreenModel_sendChatMessage(void* _this, TextHolder* text) {
	...
	if (*message == cmdMgr->prefix) {
		...
		std::call_once(flag, [] {
			Game.getClientInstance()->getGuiData()->displayClientMessageF("%sYour  prefix is: \"%s%c%s\"", RED, DARK_BLUE, cmdMgr->prefix, RED);
...
float* Hooks::Dimension_getFogColor(Dimension* _this, float* color, __int64 a3, float a4) {
	...
	static auto nightMod = moduleMgr->getModule<Freecam>();
...
float Hooks::Dimension_getTimeOfDay(Dimension* _this, int a2, float a3) {
	...
	static auto timeChange = moduleMgr->getModule<RainbowSky>();
...
float Hooks::Dimension_getSunIntensity(Dimension* a1, float a2, Vec3* a3, float a4) {
	...
	static auto nightMod = moduleMgr->getModule<RainbowSky>();
	if (nightMod->isEnabled()) {
		return -0.5f;
	}
...
__int8* Hooks::BlockLegacy_getLightEmission(BlockLegacy* a1, __int8* a2) {
	...
	static auto xrayMod = moduleMgr->getModule<RainbowSky>();
